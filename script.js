  ...
  for (const [subject, subjectLink] of Object.entries(subjectLinks)) {
    try {
      if (
        ...
        // MP requires all attendance to be marked with mark as done button
        if (subject == "MP" || subject == "DM") {
          console.log("Marking for MP")
          await mpMark(page, bot, subject);
        }
        else {// await page.waitForSelector(".statuscol");
...
const mpMark = async (page, bot, subject) => {

  await page.waitForSelector(".modtype_forum");
  const buttons = await page.$$('button[class*="btn-outline-secondary"]')
  ...
  for (const button of buttons) {
    ...
    try {
      ...
      // console.log("Dumb Hidden button, its okay")
    }
  }
  ...
  const currButtons = await page.$$('button[class*="btn-outline-secondary"]')
  var markedButtons = buttons.length - currButtons.length;

  if (markedButtons > 0 && markedButtons != 10) {
    bot.sendMessage(process.env.CHAT_ID, markedButtons + " Buttons marked for " + subject);
  }
